...
| Raster_Berechnen | Bool | Basis+Pitch auf alle Fächer anwenden (Flanke, nur Einricht) |
...
| Pitch_X | Real | Spaltenabstand (0 = Wert aus gldb.Raster behalten) |
| Pitch_Z | Real | Ebenenabstand (0 = Wert aus gldb.Raster behalten) |
| Spalten | Int | Spaltenanzahl (0 = DB, Standard 6) |
| Spalten_Rechts | Int | Spalten rechtes Regal → RegalSeite=1 (0 = DB, Standard 3) |
...
| Fehler | Bool | Fehler aktiv |
| Fehlercode | UInt | 101=ungültige Fachnr, 102=Pos nicht erlaubt, 103=Raster nicht erlaubt |
...
| R_TRIG_Raster | R_TRIG | Flanke Raster berechnen |
| instOffset | FB_Berechn_Offset | MOD/DIV pro Fach |
...
| RFID_Bereit | Bool | RFID-Leser bereit (`FB_RFID_ReadWrite` later, today = TRUE) |
...
| RFID_Code | UDInt | Gelesener RFID-Code → Fachaktuell.RFID_CODE |
...
| Raster | UDT_Lager_Raster | Basis/Pitch/Spalten — see RASTER_TEACH.md |

### UDT_Lager_Raster (in gldb_LagerverwaltungData.Raster)
| Name | Typ | Beschreibung |
|---|---|---|
| Basis_X / Basis_Z | Real | Fach 1 position |
| Pitch_X / Pitch_Z | Real | Column / level distance |
| Spalten | Int | Columns along X (default 6) |
| Spalten_Rechts | Int | Right-rack columns → RegalSeite=1 |
| Max_Fach | Int | 54 |
| Raster_Gueltig | Bool | Set after HMI_Raster_Berechnen |
...
### UDT_Fach (Felder) — see `scl/Zone_5b_Hochregallager/UDT_Fach.udt.txt`
| Name | Typ | Hinweis |
|---|---|---|
| Fachnummer | USInt | 1..54 |
| Materialart | USInt | 1 Plastic, 2 Metal |
| Color_Code | USInt | 1 Blue, 2 Green, 3 Mixed |
| Artikelnummer | UInt | YY×1000+Mat×100+Color |
| ProductTyp | UInt | Lid×10+Base |
| Kennzeichnung | String[40] | |
| Datum eingelagert | String[30] | |
| Datum_Ziffer | DTL | |
| RFID_CODE | UDInt | pallet tag stamp |
| Belegt / Gesperrt | Bool | |
| Position_X / Z | Real | |
| RegalSeite | USInt | |
| Inf_Text | String[100] | |

Full HMI/Lager UDTs: `UDT_AktuellerFach_HMI.udt.txt`, `UDT_Lager.udt.txt`, `UDT_README.md`.
